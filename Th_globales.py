...
for cell_line in lista_cell_lines:
    ...
    lista_modelos = ['P05_P90', 'P05_P80', 'P05_P70', 'P05_P65', 'P10_P90', 'P10_P80', 'P10_P70', 'P10_P65', 'P15_P90', 'P15_P80', 'P15_P70', 'P15_P65', 'Frecuencia']
    ...
    print('-------DANDO CONFIDENCIAS A LAS RXS, USANDO DOS THRESHOLD GLOBALES-------')

    df_gpr_media = df_gpr.mean(axis=1)
    lista_th_lower = {'P05': df_gpr_media.quantile(0.05), 'P10': df_gpr_media.quantile(0.10), 'P15': df_gpr_media.quantile(0.15)}
    lista_th_upper = {'P90': df_gpr_media.quantile(0.90), 'P80': df_gpr_media.quantile(0.80), 'P70': df_gpr_media.quantile(0.70), 'P65': df_gpr_media.quantile(0.65)}

    
    folder_output = './thresholding/Th_upper_y_lower/' + cell_line
    if not os.path.exists(folder_output):
        os.mkdir(folder_output)

    fname = folder_output + '/p_valor_globales.txt'
    fname2 = folder_output + '/Number_reactions_globales.txt'
            ...
            for th_l in lista_th_lower:
                for th_u in lista_th_upper:
                    ...
                    for r in reference_model.reactions:
                        if r.id in rxns_mapped_expr:
                            if rxns_mapped_expr[r.id] >= lista_th_upper[th_u]:
                                rxns_conf[r.id] = 3          
                            elif rxns_mapped_expr[r.id] <= lista_th_lower[th_l]:
                                rxns_conf[r.id] = -1 
                            # Si no hay información de algún gen, como de los mitocondriales.
                    ...
                    write_sbml_model(csm2, './thresholding/Th_upper_y_lower/' + cell_line + '/csm2_%s_%s.sbml' % (th_l, th_u))
                    ...
                    print('Numero de genes esenciales que están en CERES:', str(len(ceres_genes_linea.loc[essential_in_ceres][ceres_genes_linea>-0.2])))
                    print('Numero de genes no esenciales que están en CERES:', str(len(ceres_genes_linea.loc[essential_in_ceres])))
                    ...
                    line = th_l + '_' + th_u + '\t' + str(total_rx) + '\t'+ str(Number_essential) + '\t' + str(valor_ceres_suma)+ '\n'
                    Number_reactions.write(line)
                    
                    print('Guardado p-valor')
                    line = th_l + '_' + th_u + '\t' + str("%.4e" % p) + '\t'+ str(mean_ScoreCeres_predict) + '\t'+ str(mean_Expr_predict) + '\n'
                    p_valor.write(line)


                    print('Guardando gŕafico')
                    ...
                    sns.despine()
                    sns.distplot(ceres_genes_linea.loc[essential_in_ceres], kde=True, color='red', ax=ax1).set_title(th_l+'_'+th_u, size= 18, loc='center', fontweight="bold")
                    ...
                    fig_ruta = './thresholding/Th_upper_y_lower/' + cell_line + '/grafico_%s_%s.png' % (th_l, th_u)
                    fig.savefig(fig_ruta)
            df_frecuencia_esenciales.to_csv('./thresholding/Th_upper_y_lower/' + cell_line + '/df_frecuencia_esenciales.csv', index=True, header=True)
